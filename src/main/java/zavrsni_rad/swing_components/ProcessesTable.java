...
public class ProcessesTable  extends JFrame {
	...
	protected void initGUI() throws IOException {
		...
		filterButton.addActionListener((e) -> {
			...
			try {
				fileContent = new ArrayList<>(Files.readAllLines(processesFile.toPath()));
			} catch (IOException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			if (!processType.equals(""))
				fileContent = fileContent.stream().filter(s -> s.split(" : ")[6].equals(processType)).collect(Collectors.toList());
			if (!statusType.equals(""))
				fileContent = fileContent.stream().filter(s -> s.split(" : ")[7].equals(statusType)).collect(Collectors.toList());
			
			String[][] newData = new String[fileContent.size()][8];
			int index = 0;
			for (String s : fileContent) {
				String[] processData = s.split(" : ");
				for (int i = 0; i < processData.length; i++)
					newData[index][i] = processData[i];
				index++;
			}
			table = new JTable(newData);*/
		});
	...
	public String getProcessEnum(String type) {
		...
		switch(type) {
			case "Minimap2 alignment": return App.PanelType.MINIMAP2_ALIGN.toString();
			case "Minimap2 mapping": return App.PanelType.MINIMAP2_MAPPING.toString();
			case "Raven": return App.PanelType.RAVEN.toString();
			case "Ram mapping": return App.PanelType.RAM_MAPPING.toString();
			default: return "";
		}
	}
	...
	public String getStatusEnum(String type) {
		...
		switch(type) {
			case "Finished": return ProcessRunner.ProcessStates.FINISHED.toString();
			case "Running": return ProcessRunner.ProcessStates.RUNNING.toString();
			case "Failed": return ProcessRunner.ProcessStates.FAILED.toString();
			default: return "";
		}
	}
